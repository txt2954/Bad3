...
export class NotebookChatEditorController extends Disposable {
	...
	private async initNavigationImpl() {
		const diffInfo = this.entry.get()?.diffInfo.get();
	...
	private getDiffAssociatedWithCell(cellIndex: number) {
		return this.entry.get()?.diffInfo.get().filter(d => d.type === 'modified').find(d => d.modifiedCellIndex === cellIndex);
	}
	...
	private revealNextPrevious(cell: URI, next: boolean, strict?: boolean): boolean {
		...
		const nextCellWithChange = next ? this.entry.get()?.diffInfo.get()?.find(d => d.type === 'modified' && d.modifiedCellIndex > info.cellIndex) :
			this.entry.get()?.diffInfo.get().slice().reverse().find(d => d.type === 'modified' && d.modifiedCellIndex < info.cellIndex);
